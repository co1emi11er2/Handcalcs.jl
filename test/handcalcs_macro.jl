...
expected = LaTeXString("\\begin{align*}
Iy = \\frac{h \\cdot b^{3}}{12} = \\frac{10 \\cdot 5^{3}}{12} = 104.1667\\text{  }(\\text{moment of inertia y})
\\\\[10pt]
Ix = \\frac{b \\cdot h^{3}}{12} = \\frac{5 \\cdot 10^{3}}{12} = 416.6667\\text{  }(\\text{moment of inertia x})
\\end{align*}")
...
expected2 = LaTeXString("\\begin{align*}
z &= x \\cdot y = \\left[
\\begin{array}{c}
1 \\\\
2 \\\\
